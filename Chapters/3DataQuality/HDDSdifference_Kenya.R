library(dplyr)
library(ggplot2)
source('ImpLite_Kenya/SF_ImpLite_DietDiversity_Wote.R', chdir=T)
HDDSWote <- HDDS
source('ImpLite_Kenya/SF_ImpLite_DietDiversity_Nyando.R', chdir=T)
HDDS <- rbind(HDDS, HDDSWote)
#HDDS <- HDDS[!is.na(HDDS$foodGroup),]
source('Rhomis_Kenya/Analysis_HDDS_Kenya.R', chdir=T)
HDDSb <- subset(HDDS, household.id..see.mainsurveyinfo. %in% HDDS2015goodLong$householdID)
HDDSb$foodGroup <- gsub("fruit", "Fruits", HDDSb$foodGroup)
HDDSb$foodGroup <- gsub("eggs", "Eggs", HDDSb$foodGroup)
...
HDDSb$foodGroup <- gsub("Cereals_roots_tubers", "CerealsRootsTubers", HDDSb$foodGroup)
